...
Functional coverage is measured explicitly: `axi_top_external_line_functional_coverage` drives one comprehensive scenario and records a cover-point set — link bring-up reaching `started`/`connecting`/`running`, recovery after an error, all four link errors (`errdisc`/`errpar`/`erresc`/`errcred`), received data character, EOP, EEP, and TimeCode — then prints a coverage report (`covered 12/12 cover-points`) and fails if any point is missed, in both Verilog and VHDL. This is cover-point (functional) coverage, which the installed simulators support. RTL line/branch/toggle coverage is not collected: it requires Verilator (`--coverage`) or a gcov-enabled GHDL build, neither of which is in the current toolchain (Icarus has no coverage support and the installed GHDL uses the mcode backend). Adding a Verilator coverage job is the natural way to get line/toggle metrics once CI runs.

...
Functional coverage is measured explicitly: `axi_top_external_line_functional_coverage` drives one comprehensive scenario and records a cover-point set — link bring-up reaching `started`/`connecting`/`running`, recovery after an error, all four link errors (`errdisc`/`errpar`/`erresc`/`errcred`), received data character, EOP, EEP, and TimeCode — then prints a coverage report (`covered 12/12 cover-points`) and fails if any point is missed, in both Verilog and VHDL. This is cover-point (functional) coverage, which the Icarus/GHDL flow supports. RTL line coverage is measured separately with Verilator: `python3 tests/cocotb/verilator_coverage.py` builds the `spw_axi_top` testbench with `--coverage` across three configurations (generic loopback, external-line error injection, and fast RX/TX front ends), runs the cocotb regressions against each, and merges the result with `verilator_coverage`. The current suite reaches ~96% line coverage of the core and AXI wrappers — the data-path modules `spwxmit`/`spwxmit_fast`/`spwram`/`syncdff`/`spwrecvfront_generic`/`spw_axis_tx`/`spw_axis_rx` at 100%, `spwrecv` 99%, `spwstream` 94%, `spwlink` 93%, `spwrecvfront_fast` 90%, `spw_axi_lite_regs` 88% — with the uncovered remainder being mostly defensive `default`/error branches and compile-time range-check arms. This flow needs Verilator >= 5.022 on PATH (Icarus has no coverage support and the installed GHDL uses the mcode backend); it is not yet wired into CI because the distribution apt Verilator (5.020) predates the VPI entry points cocotb's Verilator harness calls, so it currently needs a `stable` Verilator built from source.
